Namespace SubContractingPO
    ...
    Public Class clsTable
        ...
        Public Sub FieldCreation()
            ...
            'Costing()
            GeneralSettings()
            SubContracting_Invoice()
            AddFields("OITM", "SubCont", "Sub Contract", SAPbobsCOM.BoFieldTypes.db_Alpha, 3, , , , , {"Y,Yes", "N,No"})
            AddFields("CUFD", "Valid", "Valid Flag", SAPbobsCOM.BoFieldTypes.db_Alpha, 3, , , , , {"Y,Yes", "N,No"})
            ...
            AddTables("PRICEHEAD", "PRICEHEAD", SAPbobsCOM.BoUTBTableType.bott_MasterData)
            AddTables("PRICELINE", "PRICELINE", SAPbobsCOM.BoUTBTableType.bott_MasterDataLines)
            AddFields("@PRICEHEAD", "VenName", "Vendor name", SAPbobsCOM.BoFieldTypes.db_Alpha, 50)

            AddFields("@PRICELINE", "ServiceCode", "Service Code", SAPbobsCOM.BoFieldTypes.db_Alpha, 50)
            AddFields("@PRICELINE", "ServiceName", "Service Name", SAPbobsCOM.BoFieldTypes.db_Alpha, 50)
            AddFields("@PRICELINE", "ItemCode", "Item Code", SAPbobsCOM.BoFieldTypes.db_Alpha, 50)
            AddFields("@PRICELINE", "ItemName", "Item Name", SAPbobsCOM.BoFieldTypes.db_Alpha, 50)
            AddFields("@PRICELINE", "Process", "Process", SAPbobsCOM.BoFieldTypes.db_Alpha, 50)
            AddFields("@PRICELINE", "Price", "Price", SAPbobsCOM.BoFieldTypes.db_Float, 6, SAPbobsCOM.BoFldSubTypes.st_Price)
            AddFields("@PRICELINE", "EffDate", "Effective Date", SAPbobsCOM.BoFieldTypes.db_Date)
            AddFields("@PRICELINE", "Weight", "Weight", SAPbobsCOM.BoFieldTypes.db_Float, 6, SAPbobsCOM.BoFldSubTypes.st_Measurement)

            AddUDO("PRICESUB1", "PRICESUB1", SAPbobsCOM.BoUDOObjType.boud_MasterData, "PRICEHEAD", {"PRICELINE"}, {"Code", "Name"}, True, False)

            AddTables("MI_ITMRND", "Sub-Con Item Round Master", SAPbobsCOM.BoUTBTableType.bott_NoObject)
            AddFields("@MI_ITMRND", "InvUOM", "Inventory UOM", SAPbobsCOM.BoFieldTypes.db_Alpha, 15)
            AddFields("@MI_ITMRND", "Roundoff", "Round Off", SAPbobsCOM.BoFieldTypes.db_Alpha, 3, , , "N", True)

            AddFields("WTR1", "S_DocEntry", "Sub-Con DocEntry", SAPbobsCOM.BoFieldTypes.db_Alpha, 10)
            'AddFields("WTR1", "S_LineNum", "Sub-Con LineNum", SAPbobsCOM.BoFieldTypes.db_Alpha, 10)
        End Sub
...
#Region "Document Data Creation"
        ...
        Private Sub SubContracting_Invoice()
            AddTables("SUBINV", "SubContracting Invoice Header", SAPbobsCOM.BoUTBTableType.bott_Document)
            AddTables("SUBINV1", "SubContracting Invoice Lines", SAPbobsCOM.BoUTBTableType.bott_DocumentLines)

            AddFields("@SUBINV", "BPCode", "Vendor Code", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            AddFields("@SUBINV", "BPName", "Vendor Name", SAPbobsCOM.BoFieldTypes.db_Alpha, 100)
            AddFields("@SUBINV", "DocDate", "Document Date", SAPbobsCOM.BoFieldTypes.db_Date)
            AddFields("@SUBINV", "TranEntry", "Transaction Entry", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)

            AddFields("@SUBINV1", "SDocNum", "Sub-Con Num", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            AddFields("@SUBINV1", "SDocEntry", "Sub-Con Entry", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            AddFields("@SUBINV1", "SDate", "Sub-Con Date", SAPbobsCOM.BoFieldTypes.db_Date)
            AddFields("@SUBINV1", "ItemCode", "Item Code", SAPbobsCOM.BoFieldTypes.db_Alpha, 50)
            AddFields("@SUBINV1", "ItemName", "Item Name", SAPbobsCOM.BoFieldTypes.db_Alpha, 100)
            AddFields("@SUBINV1", "SQty", "Sub-Item Qty", SAPbobsCOM.BoFieldTypes.db_Float, 10, SAPbobsCOM.BoFldSubTypes.st_Quantity)
            AddFields("@SUBINV1", "GRNNum", "Receipt Num", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            AddFields("@SUBINV1", "GRNDate", "Receipt Date", SAPbobsCOM.BoFieldTypes.db_Date)
            AddFields("@SUBINV1", "GRNQty", "Receipt Qty", SAPbobsCOM.BoFieldTypes.db_Float, 10, SAPbobsCOM.BoFldSubTypes.st_Quantity)
            AddFields("@SUBINV1", "PoEntry", "PO Entry", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            AddFields("@SUBINV1", "PoLine", "PO Line", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            AddFields("@SUBINV1", "PoDate", "PO Date", SAPbobsCOM.BoFieldTypes.db_Date)
            AddFields("@SUBINV1", "GrpoNum", "Grpo Num", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            AddFields("@SUBINV1", "GrpoDat", "Grpo Date", SAPbobsCOM.BoFieldTypes.db_Date)

            AddUDO("SCINV", "SubContractingPO", SAPbobsCOM.BoUDOObjType.boud_Document, "SUBINV", {"SUBINV1"}, {"DocEntry", "DocNum", "U_BPCode", "U_DocDate"}, True, True)

            AddFields("OIGN", "SubInv", "Sub-Rec Inv Entry", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            AddFields("POR1", "SubInv", "Sub-Rec Inv Entry", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            AddFields("@MIPL_POR2", "SubInv", "Sub-Rec Inv Entry", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)

            AddFields("OIGN", "APInv", "AP Inv Entry", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            AddFields("POR1", "APInv", "AP Inv Entry", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            AddFields("@MIPL_POR2", "APInv", "AP Inv Entry", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)

            AddFields("PCH1", "GRNEntry", "Receipt Entry", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            AddFields("OPDN", "SubGR", "Sub-Con GR Entry", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
        End Sub
        ...
        Private Sub SubContractingPO()
            ...
            AddFields("@MIPL_OPOR", "VersionNum", "Version Num", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            ...
            AddFields("@MIPL_OPOR", "ProcSeq", "Process Sequence", SAPbobsCOM.BoFieldTypes.db_Alpha, 15)
            AddFields("@MIPL_OPOR", "OwnerC", "Owner Code", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            AddFields("@MIPL_OPOR", "OwnerN", "Owner Name", SAPbobsCOM.BoFieldTypes.db_Alpha, 100)
            AddFields("@MIPL_OPOR", "OScrapQty", "Scrap Open Qty", SAPbobsCOM.BoFieldTypes.db_Float, 10, SAPbobsCOM.BoFldSubTypes.st_Quantity)
            ...
            AddFields("@MIPL_POR1", "ActPlanQty", "Actual Planned Quantity", SAPbobsCOM.BoFieldTypes.db_Float, 10, SAPbobsCOM.BoFldSubTypes.st_Quantity)
            ...
            AddFields("@MIPL_POR1", "RevBase", "Reverse BaseQty", SAPbobsCOM.BoFieldTypes.db_Alpha, 30, , , "N", , {"Y,Yes", "N,No"})
            AddFields("@MIPL_POR1", "BOMQty", "BOM Quantity", SAPbobsCOM.BoFieldTypes.db_Float, 10, SAPbobsCOM.BoFldSubTypes.st_Quantity)
            ...
            AddFields("@MIPL_POR2", "Weight", "Weight", SAPbobsCOM.BoFieldTypes.db_Float, 6, SAPbobsCOM.BoFldSubTypes.st_Measurement)
            AddFields("@MIPL_POR2", "LTType", "LineTotal Type", SAPbobsCOM.BoFieldTypes.db_Alpha, 30, , , "1", , {"1,Default", "2,Weight"})
            AddFields("@MIPL_POR2", "ActualWt", "Actual Weight", SAPbobsCOM.BoFieldTypes.db_Float, 6, SAPbobsCOM.BoFldSubTypes.st_Measurement)
...
#Region "Master Data Creation"
        ...
        Private Sub GeneralSettings()
            ...
            AddFields("@MIPL_GEN", "PType", "Process Type", SAPbobsCOM.BoFieldTypes.db_Alpha, 3, , , "N", True)
            ...
            AddFields("@MIPL_GEN", "ProcVal", "Process Validation", SAPbobsCOM.BoFieldTypes.db_Alpha, 3, , , "N", True)
            AddFields("@MIPL_GEN", "RevPlanQty", "Reverse PlanQty", SAPbobsCOM.BoFieldTypes.db_Alpha, 3, , , "N", True)
            AddFields("@MIPL_GEN", "UDOName", "UDO Name", SAPbobsCOM.BoFieldTypes.db_Alpha, 15)

            AddFields("@MIPL_GEN", "GIFIFO", "Goods Issue FIFO", SAPbobsCOM.BoFieldTypes.db_Alpha, 3, , , "N", True)
        ...
        Private Sub SubContractingBOM()
            ...
            AddFields("@MIPL_OBOM", "VersionNum", "Version Num", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            ...
            AddFields("@MIPL_OBOM", "ProcType", "Process Type", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
            ...
            AddFields("@MIPL_OBOM", "Active", "Active", SAPbobsCOM.BoFieldTypes.db_Alpha, 3, , , "Y", True)
            AddFields("@MIPL_OBOM", "ProcCode", "Process Code", SAPbobsCOM.BoFieldTypes.db_Alpha, 30)
