--------------SP for Dummy Entry Restriction --------------
if @object_type='SUBPO' and (@transaction_type ='A')
begin
	if exists
	(select  t0.DocEntry  from "@MIPL_POR1" t0	where convert(varchar,DocEntry) = @list_of_cols_val_tab_del and t0.U_Itemcode is null
	)
	begin 
		set @error = -10001
		set @error_message ='You Cannot Create a Dummy Transaction...'
